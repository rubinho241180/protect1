...
if (isset($_GET["email"])) {
	...
	$mail = "%".$_GET["email"]."%";
	// $mail = $_GET["email"];
	$andMail = " AND ((i.mac_id LIKE '".$mail."') OR (c.name LIKE '".$mail."') OR (c.email LIKE '".$mail."') OR (CONCAT(c.ddi, c.phone) LIKE '".$mail."'))";
	 // echo $andMail;
} else {
	$andMail = "";
}
...
$json = 
	array(
		"session" => array("id"=>$usr["session_id"]),
		"version" => 
			array(
				"int"=>10500,
				"str"=>"1.5.0",
...
$pdo->query("
	...
	left join
		usr u on u.id = i.chat_user_id	
	where
		i.deleted = 0 
		/*and i.cus_id in (1) */
		/*AND c.email = 'gustavo.nardi@magalhaesmedeiros.com.br'*/
	    ".$andMail." ".$andUsr."
	order by
		i.id desc	
	LIMIT 100	
...
while ($row = $qry->fetch()) { 
	...
	"select 
		...
		p.name as par_name,
		u.name as usr_name,
		(select id from seri where ser_id = s1.id LIMIT 1) as ser_recycled,
		(select ins_id from seri where ser_id = s1.id  LIMIT 1) as ser_recycled_ins_id,
	...
	while ($ro2 = $qr2->fetch()) { 
		...
		$liquid = $ro2->price-$ro2->discount;
		$dlimit = ($ro2->dlimit == NULL) ? NULL : strtotime($ro2->dlimit);
		$time   = strtotime(date('Y-m-d 00:00:00', time()));

		$expired  = (($dlimit != NULL) && (dateDiff($time, $dlimit) < 0));
		//_v1_ $blocked  = $ro2->blocked == 1;
		//_v2_ $blocked  = $ro2->_v2_disabled == 'BLOCKED'; //_v2_
		$blocked  = $ro2->blocked_at != NULL; //_v2_
		$reseted  = $ro2->ser_recycled != NULL;
		$recycled = $ro2->ser_id != NULL;
		$recneed  = (($liquid > 1.00) && (dateDiff(strtotime($ro2->timestamp), $time) > 0) && ($ro2->rectota < $liquid)); 
		$recfail  = $ro2->recfail_count > 0; 
		$enabled  = ((!$expired) && (!$blocked) && (!$reseted) && (!$recneed) && (!$recfail));

		$sql = "select CONCAT(mac_id, '-', appl_id) as ikey from ins where id = :id";
		//echo $sql;
		$qr3 = $pdo->prepare($sql);
	...
	union
	select info, timestamp from seri WHERE ins_id = $row->id
	order by 2 desc";
	...
	array_push(
		...
		array(
			...
			"chat_user_name"	=>$row->chat_user_name,
			"cus_id"	=>$row->cus_id,
