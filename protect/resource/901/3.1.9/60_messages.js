function adicionarNonoDigito(numero) {
  ...
  // 55 + DDD(2) + 8 dígitos = 12 dígitos → precisa adicionar 9
  if (telefone.length === 12) {
    const pais = telefone.slice(0, 2);      // 55
    const ddd = telefone.slice(2, 4);       // DDD
    const numeroLocal = telefone.slice(4);  // 8 dígitos

    return pais + ddd + '9' + numeroLocal;
  }
...
WAPP.serializeMsg = async function(msg)
{
    ...
    if ((!msg.isGroupMsg) && (!msg.__x_id.fromMe) && msg.senderObj.__x_id._serialized.includes('@lid'))
        senderUid = (await WPP.contact.getPnLidEntry(msg.senderObj.__x_id._serialized)).phoneNumber._serialized; else
        senderUid = msg.senderObj.__x_id._serialized;

        console.log('senderUid', senderUid);

    let senderAddress = adicionarNonoDigito(senderUid.split('@')[0]);
    // console.log('msg: ', msg);
    ...
    var cmdMsg =
    {
        ...
        sender      : {
            address : senderAddress, // msg.senderObj.__x_id.user,
            name    : (msg.senderObj.__x_pushname || msg.senderObj.__x_name || msg.senderObj.__x_formattedName),
            uid     : senderUid, //  msg.senderObj.__x_id._serialized,
        },
...
WAPP.monitoringMessages = function()
{
    WPP.whatsapp.MsgStore.off('add').on('add', (msg) => {
        ...
        if ((msg.__x_type == 'chat') && (msg.__x_t > WAPP.loggedAt)) 
        {
