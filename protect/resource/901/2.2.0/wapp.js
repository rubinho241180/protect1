    ...
    (function () {
        function getStore(modules) {
            ...
            let neededObjects = [
                ...
                { id: "WapQueryMD", conditions: (module) => (module.queryExists && module) ? module : null },
...
window.WAPI.checkNumberStatus = function(id, done) {
   ...
   // alert('will');
    ...
    //window.Store.WapQuery.queryExist(id).then((result) => {
    window.Store.WapQuery.queryPhoneExist(id).then((result) => {
        console.log('cc', result);
        if (done !== undefined) {
            //if (result.jid === undefined) throw 404;
            if (result.jid === undefined) {
                console.log('opsss_404: ' + tempId);
            }

            console.log(tempId + "_---=>_" + result.status);
            done(window.WAPI._serializeNumberStatusObj(result));
        }
    }).catch((e) => {
        console.log('opss');
