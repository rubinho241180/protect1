STATUS = {
    ...
    points: {
        ...
        disconnected: {
            has: [],
            not: [],
            status: "stUnlogged",//"stDisconnected",
            key: "disconnected"
        },
        syncing: {
            has: [],
...
STATUS.start = function(points, events = {})
{
    ...
    setInterval(function() {
        ...
        STATUS.watches[STATUS.status].forEach(function(e, i)
        {
            ...
            /*
            */
            //console.log(fhas+': '+document.querySelectorAll(fhas).length + ', ' + fhas.split(',').length + ', '+ fnot + ': '+document.querySelectorAll(fnot).length);
            //if (($(fhas).length == fhas.split(',').length) && ($(fnot).length == 0))
            if ((mySelectorAll(fhas).length == fhas.split(',').length) && (mySelectorAll(fnot).length == 0))
            {
                STATUS.status = e.key;
