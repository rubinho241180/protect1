...
function waitFound(aselector, callback, callbackDelay, execWhile) {


    // if (typeof aselector == 'object') {
    //     let selector = aselector.selector;
    // } else {
    //     let selector = aselector;
    // }

    let lsel = aselector.sel || aselector;
    let lmin = aselector.min || aselector.len || 1;
    let lmax = aselector.max || aselector.len || 999;

    console.log('waitFound: ' + lsel + ', min: '+lmin+', max: '+lmax);

    let intervalInt = mySelectorAll(lsel).length == 0 ? 100 : 0;

    var int2 = 
    setInterval(function() {

        let result = mySelectorAll(lsel);

        if (result.length >= lmin && result.length <= lmax) {
            clearInterval(int2);
            console.log('waitFound: ' + lsel + ' (' + result[0].classList.value + ') - YES! ('+result.length+')');
...
function whileFoundSendKeys(el, callback) 
{
    waitNotFound(el, callback, 0, () => 
    {
