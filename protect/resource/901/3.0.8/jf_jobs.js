...
{
    JOBS.delay = ADelay;
    JOBS.sel = job.click || job.clickTry || job.waitFound || job.waitNotFound || job.writeInput || job.style || job.sendKey || job.whileFoundSendKeys || job.length || JOBS.sel;
   ...
   //console.log('will: '+JOBS.sel);
    ...
    if ('whileFoundSendKeys' in job)
    {
        whileFoundSendKeys(JOBS.sel, () => {
    ...
    if ('waitFound' in job)
    {
        ...
        let lsel = {
            sel: JOBS.sel, 
            min: job.min || job.len || 1,
            max: job.max || job.len || 999
        }
