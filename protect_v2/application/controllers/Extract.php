...
class Extract extends CI_Controller {
	...
	public function index()
	{
		...
		/*RECEBIDO ANTECIPADO RUBINHO*/
		$sql = 'SELECT '; 
		$sql.= '  sum(r.value-r.discount-r.gateway_fee) as total '; 
		$sql.= 'FROM ';
		$sql.= '  rechist  r '; 
		$sql.= 'INNER JOIN ';
		$sql.= '  seri s ON s.id = r.seri_id ';    
		$sql.= 'WHERE '; 
    	$sql.= '  s.usr_id = 1 AND ';
		$sql.= '  r.recmethod_id = ? AND '; 
		$sql.= '  r.confirmed >  (SELECT timestamp FROM draw WHERE usr_id = ? ORDER BY id DESC LIMIT 1)';
	    //var_dump($sql);

		$stmt = $this->pdo2->pdo_connect()->prepare($sql);
		$stmt->execute(array(11, 1));
		$row = $stmt->fetch();

		//var_dump($row);

		$data['antecipado'] = $row->total;
